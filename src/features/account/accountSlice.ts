...
export const loginUser = createAsyncThunk<User, string|Object>(
    ...
    async (data, thunkAPI) => {
        try {
            ...
            //this must be strigify otherwise it gets split wiht a '.' and causes login issues.
            localStorage.setItem("jwt_soccerworld", JSON.stringify(token));
...
export const getCurrentUser = createAsyncThunk<void>(
  ...
  async(_, thunkAPI) => {
    try{
      ...
      const token = JSON.parse(localStorage.getItem("jwt_soccerworld")!);
      if (!token) {
...
export const logOutUser = createAsyncThunk<void>(
  ...
  async (_, thunkAPI) => {
    try {
      localStorage.removeItem("jwt_soccerworld");
      thunkAPI.dispatch(setLoggedIn(false));
